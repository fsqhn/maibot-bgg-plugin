...
logger = logging.getLogger("bgg_search_plugin.web_client")
...
async def bgg_search_by_name(
    ...
    try:
        ...
        if verbose: logger.info(f"[BGG 网页搜索] 正在搜索: {query}")
        ...
        for a_tag in soup.find_all("a", href=True):
            href = a_tag.get("href", "")
            if ("/boardgame/" in href and "/boardgamegame" not in href and "/boardgameexpansion" not in href):
                parts = href.split("/")
                if len(parts) >= 3:
                    try:
        ...
        if verbose: logger.info(f"[BGG 网页搜索] 找到游戏: ID={bgg_id}, 名称={game_name}")
...
async def bgg_thing_details(
    ...
    try:
        if verbose: logger.info(f"[BGG 详情] 访问: {game_url}")
        ...
        return {
            "bgg_id": str(item.get("objectid", "")), "name": game_name_en,
            "year": str(item.get("yearpublished", "")), "description": description,
            "min_players": str(item.get("minplayers", "?")), "max_players": str(item.get("maxplayers", "?")),
            "min_time": str(item.get("minplaytime", "?")), "max_time": str(item.get("maxplaytime", "?")),
            "min_age": str(item.get("minage", "?")), "users_rated": "0", "average": "0",
            "avg_weight": avgweight_str, "rank": str(overall_rank), "strategy_rank": str(strategy_rank),
            "image": game_image, "bgg_url": game_url, "categories": categories, "mechanics": mechanics,
            "best_numplayers": "", "language_dependence": "",
        }
    except Exception as e:
